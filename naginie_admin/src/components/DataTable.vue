  ...
  <table class="border-collapse w-full">
    ...
    <tbody>
      <template v-for="item in data">
        <tr
          v-bind:key="item.id"
          class="bg-grey-800 lg:hover:bg-gray-700 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0 "
        >
          <template v-for="(title, property) in headers">
            <td
              v-bind:key="`${property}_${item.id}`"
              class="w-full border border-teal-700 lg:w-auto p-3 text-gray-300  text-center  block lg:table-cell relative lg:static"
            >
              <span
